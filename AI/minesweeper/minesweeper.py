...
class MinesweeperAI():
            ...
            for c in range(j-1,j+2):
                ...
                if 0 <= r < self.height and 0 <= c < self.width:
                    ...
                    if pos in self.mines:
                        flagged_mines +=1
                    elif pos not in self.safes and pos not in self.moves_made:
                        neighbors.add(pos)

        adjusted_count = count - flagged_mines
        if neighbors:
        ...
        while chg:
            ...
            for k1 in self.knowledge:
                for k2 in self.knowledge:
                    ...
                    if k1.cells and k2.cells and k1.cells.issubset(k2.cells):
                        diff_c = k2.cells - k1.cells
                        diff_count = k2.count - k1.count
                        if diff_c:
                            candidate = Sentence(diff_c,diff_count)
                            if candidate not in self.knowledge and candidate not in new_inferences:
                                new_inferences.append(candidate)
            if new_inferences:
    ...
    def make_random_move(self):
        ...
        _random = list(all_cells - self.moves_made - self.mines)
        if not _random:
            return None
        return random.choice(_random)
