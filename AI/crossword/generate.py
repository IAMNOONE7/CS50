...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for wx in self.domains[x]:
            supported = any(wx[i] == wy[j] for wy in self.domains[y])
            if not supported:
                to_remove.add(wx)
    ...
    def assignment_complete(self, assignment):
        ...
        if len(assignment) != len(self.crossword.variables):
            return False
        return all(assignment.get(v) is not None for v in self.crossword.variables)
    ...
    def order_domain_values(self, var, assignment):
        ...
        def conflicts_for_value(w):
            ...
            for n in self.crossword.neighbors(var):
                ...
                count += sum(1 for wn in self.domains[n] if wn[j] != w[i])
            return count
        return sorted(self.domains[var], key = conflicts_for_value)
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var, assignment):
            ...
            self.domains = {v: saved_domains[v].copy() for v in saved_domains}
